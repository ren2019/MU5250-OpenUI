...
export default function DiagnosticsPage() {
  const { series, now, paused, session, thermalFailed } = useDiagnosticSession()
  ...
  return <div className="diagnostics">
    ...
    <div className="diagnostic-layout"><div>{[...throughputMetrics, ...deviceMetrics].map(metric => <DiagnosticChart key={metric.id} metric={metric} samples={series[metric.id] ?? []} start={start} end={now} cursor={cursor} onCursor={setCursor} />)}</div><aside><h3>变化时间线</h3><p>当前显示速率采样。无线与连接变化将在后续诊断功能中记录。</p><p>曲线空白表示缺失、过期或采集间断，不能据此认定设备断网。</p></aside></div>
